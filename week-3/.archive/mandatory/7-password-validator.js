...
const isValidLength = (password) => password.length >= 5;

const hasUpperCase = (password) => password !== password.toLowerCase();

const hasLowerCase = (password) => password !== password.toUpperCase();

const hasNumbers = (password) =>
  [...Array(10).keys()].filter((number) => password.includes(number)).length !==
  0;

const hasSymbols = (password) =>
  ["!", "#", "$", "%", "."].filter((symbol) => password.includes(symbol))
    .length !== 0;

function validatePasswords(passwords) {
  const passwordsSeen = [];
  return passwords.map((password) => {
    if (passwordsSeen.includes(password)) {
      return false;
    } else {
      passwordsSeen[passwordsSeen.length] = password;
      return (
